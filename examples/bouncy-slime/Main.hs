...
slimeSprite :: Picture
slimeSprite = bmp "examples/bouncy-slime/Slime.bmp"

slimeBounceSprites :: [Picture]
slimeBounceSprites =
  [ scale 0.5 0.5 $ bmp "examples/bouncy-slime/Slime2.bmp",
    scale 0.5 0.5 $ bmp "examples/bouncy-slime/Slime3.bmp",
    scale 0.5 0.5 $ bmp "examples/bouncy-slime/Slime4.bmp",
    scale 0.5 0.5 $ bmp "examples/bouncy-slime/Slime2.bmp"
  ]

platformSprite :: Picture
platformSprite = bmp "examples/bouncy-slime/Platform.bmp"
...
main =
  playInScene
    (InWindow "Bouncy Slime" (round width, round height) (50, 50))
    white
    30
    initialWorld
    level
  ...
  where
    ...
    applyMovement _ _ world
      | 'a' `elem` pressedKeys world = world {slimePos = moveX (slimePos world) (-10)}
    ...
    applyGravity now _ world
      | onTheFloor world
          || onThePlatform world =
          world
            { slimeVelocity = slimeVelocity world * (-0.5),
              slimeAnim =
                if slimeVelocity world < -5
    ...
    onThePlatform world =
      snd (slimePos world) <= snd platformPos + slimeHeight / 2 + platformHeight / 2
