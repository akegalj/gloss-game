...
slimeSprite :: Picture
slimeSprite = bmp "examples/slime/Slime.bmp"

slimeBallSprite :: Picture
slimeBallSprite = bmp "examples/slime/SlimeBall.bmp"
...
main = play (InWindow "Slime" (600, 400) (50, 50)) white 30 initialWorld draw handle [boxInSlime, moveBall]
  where
    draw (World {slimePos = (x, y), ballPos = ballPos}) =
      pictures
        [ translate x y (scale 0.5 0.5 slimeSprite),
    ...
    moveBall _ world@(World {ballPos = Just (x, y)}) =
      if x > 300
