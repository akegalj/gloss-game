...
creeperSprite = bmp "examples/bounce/Creeper.bmp"

platformSprite :: Picture
platformSprite = bmp "examples/bounce/Platform.bmp"
...
main = play (InWindow "Bouncy Creeper" (600, 400) (50, 50)) white 30 initialWorld draw handle [applyVelocity, applyGravity]
  where
    draw (World {creeperPos = (x, y)}) =
      pictures
        [ translate x y (scale 0.5 0.5 creeperSprite),
    ...
    applyGravity _ world
      | onTheFloor world
          || onThePlatform world =
          world {creeperVelocity = creeperVelocity world * (-0.5)}
    ...
    onThePlatform world =
      snd (creeperPos world) <= snd platformPos + creeperHeight / 2 + platformHeight / 2
        && fst (creeperPos world) + creeperWidth / 2 > fst platformPos - platformWidth / 2
        && fst (creeperPos world) - creeperWidth / 2 < fst platformPos + platformWidth / 2

moveX :: Point -> Float -> Point
moveX (x, y) offset =
  if x + offset > 300 - creeperWidth / 2
...
moveY (x, y) offset =
  if y + offset > 200 - creeperHeight / 2
    then (x, 200 - creeperHeight / 2)
    else
      if y + offset < (-200) + creeperHeight / 2
        then (x, (-200) + creeperHeight / 2)
        else
          if y + offset < snd platformPos + creeperHeight / 2 + platformHeight / 2
